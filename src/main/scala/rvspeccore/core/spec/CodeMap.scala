...
sealed abstract class CodeMap {
  val map: Map[String, UInt]
...
object OpcodeMap extends CodeMap {
  ...
  val map = {
    for (
      i <- 0 until rawTable.size;
      j <- 0 until rawTable(i).size
    ) yield rawTable(i)(j) -> getOpcode(i, j)
  }.toMap
}

object Funct3Map extends CodeMap {
  val map = RVInsts.funct3Map
}

object Funct7Map extends CodeMap {
  val map = RVInsts.funct7Map
}

object InstOpcodeMap extends CodeMap {
  val map = RVInsts.opcodeNameMap.map { case (instName, opcodeName) => instName -> OpcodeMap(opcodeName) }
}
