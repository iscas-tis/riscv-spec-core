    ...
    def doRaiseExceptionM(exceptionCode: UInt, MXLEN: Int): Unit = {
      ...
      switch(exceptionCode) {
        is(MExceptionCode.illegalInstruction.U) {
          ...
          next.privilege.csr.mtval := 0.U
          // when(io.inst(1, 0) =/= "b11".U(2.W)) { next.privilege.csr.mtval := io.inst(15, 0) }
          //   .otherwise { next.privilege.csr.mtval := io.inst(31, 0) }
        }
        is(MExceptionCode.instructionAccessFault.U) {
          when(inst(1, 0) =/= "b11".U(2.W)) { next.privilege.csr.mtval := inst(15, 0) }
            .otherwise { next.privilege.csr.mtval := inst(31, 0) }
        }
        is(MExceptionCode.environmentCallFromMmode.U) {
          next.privilege.csr.mtval := 0.U
        }
        is(MExceptionCode.storeOrAMOAddressMisaligned.U) {
          next.privilege.csr.mtval := mem.write.addr
          // printf("[Debug]:storeOrAMOAddressMisaligned %x %x\n",io.mem.write.addr,next.privilege.csr.mtval)
        }
        is(MExceptionCode.loadAddressMisaligned.U) {
          next.privilege.csr.mtval := mem.read.addr
          // printf("[Debug]:loadAddressMisaligned %x %x\n",io.mem.read.addr,next.privilege.csr.mtval)
        }
        is(MExceptionCode.instructionAddressMisaligned.U) {
          // next.privilege.csr.mtval := io.mem.read.addr
    ...
    def doRaiseExceptionS(exceptionCode: UInt, MXLEN: Int): Unit = {
      ...
      switch(exceptionCode) {
        is(MExceptionCode.illegalInstruction.U) {
          ...
          when(inst(1, 0) =/= "b11".U(2.W)) { next.privilege.csr.stval := inst(15, 0) }
            .otherwise { next.privilege.csr.stval := inst(31, 0) }
        }
        is(MExceptionCode.instructionAccessFault.U) {
          when(inst(1, 0) =/= "b11".U(2.W)) { next.privilege.csr.stval := inst(15, 0) }
            .otherwise { next.privilege.csr.stval := inst(31, 0) }
        ...
        is(MExceptionCode.environmentCallFromMmode.U) {
          ...
          // FIXME: 实际上已经不存在这种情况了
          // when(io.inst(1, 0) =/= "b11".U(2.W)) { next.privilege.csr.stval := io.inst(15, 0) }
          //   .otherwise { next.privilege.csr.stval := io.inst(31, 0) }
          next.privilege.csr.stval := 0.U
        }
        // FIXME:三种非对齐访存 把非必要的Case进行合并
        is(MExceptionCode.storeOrAMOAddressMisaligned.U) {
          next.privilege.csr.stval := mem.write.addr
          // printf("[Debug]:storeOrAMOAddressMisaligned %x %x\n",io.mem.write.addr,next.privilege.csr.stval)
        }
        is(MExceptionCode.loadAddressMisaligned.U) {
          next.privilege.csr.stval := mem.read.addr
          // printf("[Debug]:loadAddressMisaligned %x %x\n",io.mem.read.addr,next.privilege.csr.stval)

        }
        is(MExceptionCode.instructionAddressMisaligned.U) {
          // next.privilege.csr.stval := io.mem.read.addr
          // printf("[Debug]:instructionAddressMisaligned %x %x\n",io.mem.read.addr,next.privilege.csr.stval)
        }
      }
